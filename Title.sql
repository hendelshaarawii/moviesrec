...
SELECT url FROM movies10 WHERE lexemesTitle1 @@ to_tsquery(‘zodiac’);
 
UPDATE movies10 SET rank=ts_rank(lexemesTitle1,plainto_tsquery((SELECT title FROM movies10 WHERE url =‘zodiac’)));
